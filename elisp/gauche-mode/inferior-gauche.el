...
(setq inferior-gauche-mode-map
      (let ((map (make-sparse-keymap)))
        ...
        (define-key map (kbd "C-M-x") 'igosh-eval-define)
        (define-key map (kbd "C-x C-e") 'igosh-eval-last-sexp)
        (define-key map (kbd "C-j") 'igosh-eval-print-last-sexp)
        (define-key map (kbd "C-c C-s") 'igosh-start-process)
        (define-key map (kbd "M-C-i") 'igosh-complete-symbol)
        (define-key map (kbd "C-c C-d") 'igosh-insert-debug-print)
        (define-key map (kbd "C-c C-c") 'igosh-interrupt-process)
        (define-key map (kbd "C-c C-q") 'igosh-exit-process)
        (define-key map (kbd "C-c C-v") 'igosh-eval-buffer)
        (define-key map (kbd "C-c C-i") 'igosh-input)
        map))

;; ===========================================
;; Main Functions

(define-derived-mode inferior-gauche-mode gauche-mode
